...
private:
  ...
  DepthAnalysis analyzeDepth(const std::shared_ptr<trading::OrderBookSnapshot>& snapshot) {
    ...
    // Analyze bid side
    int bidLevels = 0;
    for (auto it = snapshot->bids.rbegin(); it != snapshot->bids.rend() && bidLevels < depthLevels;
         ++it) {
    ...
    // Analyze ask side
    int askLevels = 0;
    for (auto it = snapshot->asks.begin(); it != snapshot->asks.end() && askLevels < depthLevels;
         ++it) {
...
public:
  MarketMaker(std::shared_ptr<trading::OrderBook> ob,
      ...
      : orderBook(ob), trading(t), shutdownFlag(sf), minSpread(ms), minProfit(mp), orderSize(os),
        productId(pid), currentBidOrderId(""), currentAskOrderId(""), currentBidPrice(0),
        currentAskPrice(0) {
  }
  ...
  void checkOrderStatus() {
    ...
    if (!currentBidOrderId.empty()) {
      try {
        ...
        if (jsonResponse["status"] == "FILLED" || jsonResponse["status"] == "CANCELED") {
          std::cout << "Bid order " << currentBidOrderId << " was "
                    << jsonResponse["status"].get<std::string>() << std::endl;
          currentBidOrderId = "";
          currentBidPrice = 0;
        }
    ...
    if (!currentAskOrderId.empty()) {
      try {
        ...
        auto jsonResponse = nlohmann::json::parse(response);
        if (jsonResponse["status"] == "FILLED" || jsonResponse["status"] == "CANCELED") {
          std::cout << "Ask order " << currentAskOrderId << " was "
                    << jsonResponse["status"].get<std::string>() << std::endl;
          currentAskOrderId = "";
          currentAskPrice = 0;
        }
    ...
    while (!shutdownFlag.load()) {
      ...
      if (snapshot) {
        ...
        if (hasValidPrices && bestBid > 0 && bestAsk > 0) {
          ...
          double midPrice =
              depth.weightedMidPrice > 0 ? depth.weightedMidPrice : (bestBid + bestAsk) / 2.0;
...
private:
  void placeBidOrder(double price) {
    ...
    try {
      ...
      if (jsonResponse["success"] == true && jsonResponse.contains("order_id") &&
          !jsonResponse["order_id"].is_null()) {
        currentBidOrderId = jsonResponse["order_id"].get<std::string>();
        currentBidPrice = price;
        std::cout << "Successfully placed bid order at " << price << " with ID: " << currentBidOrderId
                  << std::endl;
      } else {
  ...
  void placeAskOrder(double price) {
    ...
    try {
      ...
      if (jsonResponse["success"] == true && jsonResponse.contains("order_id") &&
        ...
        std::cout << "Successfully placed ask order at " << price << " with ID: " << currentAskOrderId
                  << std::endl;
      } else {
...
void readerThreadFunction(std::shared_ptr<trading::OrderBook> orderBook,
                          std::atomic<bool>& shutdownFlag) {
