...
private:
  ...
  std::map<std::string, std::string> generateAuthHeaders(const std::string& method,
...
void Trading::getAccounts() {
  ...
  } catch (std::exception const& e) { std::cerr << "Error in login: " << e.what() << std::endl; }
}
...
std::string Trading::calculateSignature(const std::string& message, const std::string& secretKey) {
  ...
  HMAC(EVP_sha256(), secretKey.c_str(), (int)secretKey.size(),
       reinterpret_cast<const unsigned char*>(message.c_str()), (int)message.size(), hmacResult,
       &hmacLength);
...
std::map<std::string, std::string> Trading::generateAuthHeaders(const std::string& method,
                                                                const std::string& path,
                                                                const std::string& body) {
  std::map<std::string, std::string> headers = {{"Authorization",
                                                 "Bearer " + create_jwt(method, path)},
