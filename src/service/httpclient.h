...
private:
    ...
    void connectTLS(ssl::stream<beast::tcp_stream>& stream, const std::string& host, const std::string& port);
    std::string sendRequestAndGetResponse(ssl::stream<beast::tcp_stream>& stream, http::request<http::string_body>& req);
};
